...
/**
 * Run Python code snippet with optional arguments
 */
export async function runPythonSnippet(request: ExecRequest): Promise<ExecResult> {
    ...
    const tempFile = path.join(tempDir, `mcp_exec_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
